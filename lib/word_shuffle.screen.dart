...
class _WordShuffleState extends State<WordShuffle> {
  ...
  void initState() {
    ...
    List<String> initialScramble =
        scramble(finalWord.characters).map((e) => e.toString()).toList();
    print(initialScramble);
    tiles = {
      for (int i = 0; i < finalWord.characters.length; i++)
        i: initialScramble.elementAt(i)
    };
    tileDestinations = {
      for (int i = 0; i < finalWord.characters.length; i++) i: null
    };
  }
      ...
      child: Column(
        ...
        children: [
              ...
              child: Wrap(
                ...
                children: [
                  for (String word in words)
                    buildValidWord(word, submittedWords.contains(word))
                ],
              ),
            ),
          ),
                          ...
                          if (mounted) {
                            setState(() {
                              if (acceptDetails.data.$1 == true) {
                                tileDestinations[tileD.key] =
                                    acceptDetails.data.$2.value;
                                tileDestinations[acceptDetails.data.$2.key] =
                                    null;
                              } else {
                                tileDestinations[tileD.key] =
                                    acceptDetails.data.$2.value;
          ...
          Row(
            ...
            children: [
              IconButton(
                  onPressed: resetInput, icon: const Icon(Icons.arrow_downward))
            ],
          ),
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              IconButton(
                  onPressed: shuffleTiles, icon: const Icon(Icons.shuffle))
            ],
          ),
          Flexible(
            child: GridView.count(
          ...
          SafeArea(
            child: Container(
              ...
              child: ElevatedButton(
                ...
                child:
                    Text(currentWord().isNotEmpty ? currentWord() : "NO WORD"),
              ),
            ),
          ),
        ],
  ...
  String currentWord() =>
      tileDestinations.values.map((e) => e ?? ' ').join("").trim();
